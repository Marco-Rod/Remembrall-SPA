...
@api.route("/plan/<int:id>/", methods=["PATCH"])
@token_required
def update_plan(id):
    plan = Plan.query.get(id)
    data = request.get_json()
...
@token_required
def add_participants(current_user,id):
    plan = Plan.query.get(id)
    data = request.get_json()
    for user in data["id"]:
...
@token_required
def remove_participants_plan(current_user,participant_id, plan_id):
...
@token_required
def save_payments(current_user):
    data = request.get_json()
